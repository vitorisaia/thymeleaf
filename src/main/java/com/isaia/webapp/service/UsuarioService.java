...
import com.isaia.webapp.model.UsuarioModel;
import com.isaia.webapp.model.UsuarioSecurityModel;
import com.isaia.webapp.repository.GrupoRepository;
import com.isaia.webapp.repository.PermissaoRepository;
import com.isaia.webapp.repository.UsuarioRepository;
	...
	public void salvarUsuario(UsuarioModel usuarioModel) {
		...
		for (Long codigoGrupo : usuarioModel.getGrupos()) {
			...
			if (codigoGrupo != null) {
				...
				grupoEntity = grupoRepository.findOne(Example.of(new GrupoEntity().setCodigo(codigoGrupo)));
//				grupoEntity = grupoRepository.findOne(codigoGrupo);
	...
	public void excluir(Long codigoUsuario) {
		...
		this.usuarioRepository.delete(new UsuarioEntity().setCodigo(codigoUsuario));
	}
	...
	public UsuarioModel consultarUsuario(Long codigoUsuario) {
		...
		Optional<UsuarioEntity> usuarioEntity = this.usuarioRepository.findOne(Example.of(new UsuarioEntity().setCodigo(codigoUsuario)));

		List<Long> grupos = new ArrayList<Long>();

		usuarioEntity.get().getGrupos().forEach(grupo -> {

			grupos.add(grupo.getCodigo());

		});

		return new UsuarioModel(usuarioEntity.get().getCodigo(), usuarioEntity.get().getNome(), usuarioEntity.get().getLogin(), null,
				usuarioEntity.get().isAtivo(), grupos);
	...
	public void alterarUsuario(UsuarioModel usuarioModel) {
		...
		Optional<UsuarioEntity> _usuarioEntity = this.usuarioRepository.findOne(Example.of(new UsuarioEntity().setCodigo(usuarioModel.getCodigo())));

		UsuarioEntity usuarioEntity = _usuarioEntity.get();
		...
		for (Long codigoGrupo : usuarioModel.getGrupos()) {
			...
			if (codigoGrupo != null) {
				...
				/* CONSULTA GRUPO POR CÓDIGO */
				grupoEntity = grupoRepository.findOne(Example.of(new GrupoEntity().setCodigo(codigoGrupo))).get();
